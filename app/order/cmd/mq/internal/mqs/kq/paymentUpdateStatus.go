...
import (
	...
	paymentModel "microservices-go-zero/app/payment/model"
...
// payment update status message queue consumer
...
// consumer update the homestay order status based on payment status
func (l *PaymentUpdateStatusMq) execService(message kqueue.ThirdPaymentUpdatePayStatusNotifyMessage) error {
	...
	if orderTradeState != -99 {
		// call order rpc to update homestay order state
