...
// update homestay order status to new status
func (l *UpdateHomestayOrderTradeStateLogic) verifyOrderTradeState(newTradeState, oldTradeState int64) error {
	// only create order can change the order trade state to wait to be paied 
	...
	if newTradeState == model.HomestayOrderTradeStateCancel {
		// note: it seems this project only supports cancel unpaid order
		// note: doesn't matter, just a toy and study project
		if oldTradeState != model.HomestayOrderTradeStateWaitPay {
			return errors.Wrapf(xerr.NewErrMsg("Only orders that are pending payment can be cancelled"),
