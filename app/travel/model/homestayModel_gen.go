...
var (
	...
	cacheMicroservicesTravelHomestayIdPrefix = "cache:microservicesTravel:homestay:id:"
)
...
func (m *defaultHomestayModel) Insert(ctx context.Context, session sqlx.Session, data *Homestay) (sql.Result, error) {
	...
	microservicesTravelHomestayIdKey := fmt.Sprintf("%s%v", cacheMicroservicesTravelHomestayIdPrefix, data.Id)
	...
	}, microservicesTravelHomestayIdKey)
}

func (m *defaultHomestayModel) FindOne(ctx context.Context, id int64) (*Homestay, error) {
	microservicesTravelHomestayIdKey := fmt.Sprintf("%s%v", cacheMicroservicesTravelHomestayIdPrefix, id)
	var resp Homestay
	err := m.QueryRowCtx(ctx, &resp, microservicesTravelHomestayIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
// Return the first pageSize records (del_state = 0) in the table whose id is less than preMinId
...
// Return the first pageSize records (del_state = 0) in the table whose id is bigger than preMinId
...
func (m *defaultHomestayModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheMicroservicesTravelHomestayIdPrefix, primary)
}
